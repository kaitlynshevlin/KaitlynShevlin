...
hist_copula_ar2 <- ggplot(results) + 
                   ...
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y") +
                   labs(caption = sprintf("AR = (%s, %s)", b1, b2))
...
hist_copula_arma <- ggplot(results) + 
                   ...
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y") +
                   labs(caption = sprintf("AR = %s, MA = %s", ar, ma))
...
hist_copula_gamma <- ggplot(results) + 
                   ...
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y") +
                   labs(caption = sprintf("Rho = %s, Shape = %s, Scale = %s", rho, shape, scale))
...
# Save the histogram figure
ggsave(filename = "hist_copula_gamma.pdf", plot = hist_copula_gamma, path = "../Manuscript", height = 2.1, width = 6)
```
...
# P-vals from using bootstrap with copulas
pvals_copula <- replicate(nrep, test_gev(genData(n, qgev, loc = 0, shape = shape, scale = 1, copula = normalCopula, rho = rho), B = B))
counter <- 0

# P-vals from using parametric bootstrap
pvals_parametric <- suppressWarnings(replicate(nrep, test_parametric_gamma(genData(n, qgev, loc = 0, shape = shape, scale = 1, copula = normalCopula, rho = rho))$p.value))

# P-vals from naively using fitted parameters
pvals_naive <- replicate(nrep, test_super_naive_gamma(genData(n, qgev, loc = 0, shape = shape, scale = 1, copula = normalCopula, rho = rho))$p.value)
...
hist_copula_gev <- ggplot(results) + 
                   ...
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y") +
                   labs(caption = sprintf("Rho = %s, Loc = %s, Shape = %s, Scale = %s", rho, loc, shape, scale))
...
# Super naive KS-test with fitted parameters
test_super_duper_naive <- function(x) {
  p.value <- ks.test(x, "pnorm", mean = 0, sd = 1)$p.value
  list(p.value = p.value)
}
...
# P-vals from naively using fitted parameters
pvals_naive <- replicate(nrep, test_super_duper_naive(arima.sim(list(ar = ar), rand.gen=rnorm, sd = sqrt(1 - ar^2), n = n))$p.value)
...
# Generate histograms plot
hist_copula_only <- ggplot(results) + 
                   geom_histogram(aes(x = p_values, fill = histogram), boundary = 0, binwidth = 1/20) + 
                   scale_x_continuous(name = "p-value", breaks=c(0, 1)) + 
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y") +
...
# Save the histogram figure
ggsave(filename = "hist_copula_only.pdf", plot = hist_copula_only, path = "../Manuscript", height = 2.1, width = 6)
```