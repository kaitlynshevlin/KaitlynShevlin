...
```{r ARMA(1,1) D}
test_copula_D <- function(x, pdist, qdist, B, ...) {
    ...
    for (i in 1:B) {
        ...
        zx.b <- arima.sim(n, ...) # simulate from fit
        u <- pnorm(zx.b)
        # Transform bootstrap sample and fit the parameters
        x.b <- qdist(u, ...) # mean = mean, sd = sd)
        
        # Gather bootstrap sample's test statistic
        stat.b[i] <- ks.test(x.b, pdist, mean = mean, sd = sd)$statistic # fix
    }
