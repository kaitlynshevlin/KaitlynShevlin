...
pvals_copula <- replicate(nrep, test_copula(arima.sim(list(order = c(2, 0, 0), 
                                                          ...
                                                          n = n), "pnorm", B = B))
pvals_naive <- replicate(nrep, test_naive_copula(arima.sim(list(order = c(2, 0, 0), ar = c(b1, b2)), rand.gen=rnorm, sd = sqrt(((1+b2)*(1-b2-b1)*(1-b2+b1))/(1-b2)), n = n))$p.value)
...
b2 <- 0.3
n <- 100
x <- arima.sim(list(order = c(2, 0, 0), ar = c(b1, b2)), rand.gen=rnorm, sd = sqrt(((1+b2)*(1-b2-b1)*(1-b2+b1))/(1-b2)), n = n)
sd(x)
var(x)

ma <- 0.5
x <- arima.sim(list(ma = 0.5), rand.gen = rnorm, sd = sqrt(1/(1+ma^2)), n = n)
sd(x)
var(x)

ar <- 0.5
x <- arima.sim(list(ar = ar, ma = ma), rand.gen = rnorm, sd = sqrt((1-(ar^2))/(1+2*ar*ma+ma^2)), n = n)
sd(x)
var(x)
```
...
pvals_naive <- replicate(nrep, test_naive_copula(arima.sim(list(ma = 0.5), rand.gen = rnorm, sd = sqrt(1/(1+ma^2)), n = n))$p.value)
...
pvals_copula <- replicate(nrep, test_copula(arima.sim(list(ar = ar, 
                                                      ...
                                                      n = n),
                                            "pnorm", B = B))
pvals_naive <- replicate(nrep, test_naive_copula(arima.sim(list(ar = ar, 
                                                                ma = ma), 
                                                      rand.gen=rnorm, 
                                                      sd = sqrt((1-(ar^2))/(1+2*ar*ma+ma^2)), 
                                                      n = n))$p.value)
...
test_gamma <- function(x, dist, B = 100, wcopula = normalCopula) {
    ...
    #fit <- fitdist(x, "gamma", method = "mle", lower = c(0, 0), start = list(scale = 1, shape = 1))
    ...
    for (i in 1:B) {
        ...
        #fit.b <- fitdist(x.b, "gamma", method = "mle", lower = c(0, 0), start = list(scale = 1, shape = 1))
        #shape.b <- fit.b$estimate[1]
...
# Naive KS-test
test_naive_copula <- function(x) {
  p.value <- ks.test(x, "pgamma", shape = 3, scale = 1)$p.value
  list(p.value = p.value)
}
...
pvals_naive <- replicate(nrep, test_naive_copula(genData(n, qgamma, shape = 3, scale = 1, copula = normalCopula, rho = rho))$p.value)
...
```{r}
rho <- 0.5
x <- genData(100, qgamma, shape = 3, rate = 2, copula = normalCopula, rho = rho)
ks.test(x, "pgamma", shape = 3, rate = 2)
```
}}
