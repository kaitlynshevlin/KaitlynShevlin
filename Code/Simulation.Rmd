...
data_new <- results                              # Replicate data
data_new$histogram <- factor(data_new$histogram,      # Reordering group factor levels
                         levels = c("Naive", "Parametric", "Babu"))

# Generate histograms plot
hist_fitted <- ggplot(data_new) + 
                   geom_histogram(aes(x = p_values, fill = histogram), boundary = 0, binwidth = 1/20) + 
                   scale_x_continuous(name = "p-value", breaks=c(0, 1)) + 
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y")
...
```{r AR(1) D}
test_copula_D <- function(x, dist, B, mean, sd) {

    # Get observed statistic
    n <- length(x)

    stat  <- ks.test(x, dist, mean = mean, sd = sd)$statistic
    ...
    for (i in 1:B) {
        ...
        x.b <- qnorm(u, mean = mean, sd = sd)
        
        # Gather bootstrap sample's test statistic
        stat.b[i] <- ks.test(x.b, dist, mean = mean, sd = sd)$statistic
    }
...
# Standard KS test
test_standard_ks <- function(x, mean, sd) {
  p.value <- ks.test(x, "pnorm", mean = mean, sd = sd)$p.value
  list(p.value = p.value)
}
...
pvals_copula <- replicate(nrep, test_copula_D(arima.sim(list(ar = ar), rand.gen=rnorm, sd = sqrt(1 - ar^2), n = n), 
                                              "pnorm", 
                                              B = B, 
                                              mean = mean, 
                                              sd = sd))
...
test_gamma_D <- function(x, dist, B = 100, wcopula = normalCopula, shape, scale) {
    ...
    for (i in 1:B) {
        ...
        stat.b[i] <- ks.test(x.b, dist, shape = shape, scale = scale)$statistic
    }
...
test_standard_gamma <- function(x, shape, scale) {
...
# P-vals from using bootstrap with copulas
pvals_copula <- replicate(nrep, test_gamma_D(genData(n, qgamma, shape = shape, scale = scale, copula = normalCopula, rho = rho), 
                                             "pgamma", 
                                             B = B,
                                             shape = shape,
                                             scale = scale))
...
pvals_naive <- replicate(nrep, test_standard_gamma(genData(n, qgamma, shape = shape, scale = scale, copula = normalCopula, rho = rho), 
                                                   shape = shape, 
                                                   scale = scale)$p.value)
...
test_gev_D <- function(x, B = 100, wcopula = normalCopula, loc, scale, shape) {
    ...
    for (i in 1:B) {
        ...
        stat.b[i] <- ks.test(x.b, 'pgev', loc = loc, scale = scale, shape = shape)$statistic
    }
...
# Standard KS test for GEV distribution
test_standard_gev <- function(x, loc, scale, shape) {
...
# P-vals from using bootstrap with copulas
pvals_copula <- replicate(nrep, test_gev_D(genData(n, qgev, loc = loc, shape = shape, scale = scale, copula = normalCopula, rho = rho),
                                           loc = loc, 
                                           shape = shape,
                                           scale = scale,
                                           B = B))
...
# P-vals from standard KS test
pvals_naive <- replicate(nrep, test_standard_gev(genData(n, qgev, loc = loc, shape = shape, scale = scale, copula = normalCopula, rho = rho), 
                                                 loc = loc, 
                                                 shape = shape,
                                                 scale = scale)$p.value)
...
```{r Copula FD}
test_copula_D <- function(x, dist, B) {
...
# P-vals from using bootstrap with copulas
pvals_copula <- replicate(nrep, test_copula_D(arima.sim(list(ar = ar), rand.gen=rnorm, sd = sqrt(1 - ar^2), n = n), "pnorm", B = B))
counter <- 0
...
results <- data_frame(p_values, histogram)
save(results, file = "results_copula.RData")
...
# Save the histogram figure
ggsave(filename = "hist_ar1_D", plot = hist_ar1_D, path = "../Manuscript", height = 2.1, width = 6)
```
...
# P-vals from using bootstrap with copulas
pvals_copula <- replicate(nrep, test_copula_D(arima.sim(list(ma = ma), rand.gen = rnorm, sd = sqrt(1/(1+ma^2)), n = n), "pnorm", B = B))
counter <- 0
