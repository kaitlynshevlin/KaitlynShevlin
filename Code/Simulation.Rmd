...
```{r babu.ts}
ks.np.babu.ts <- function(x, l, B = 1000) {
  ## for normal distribution only
  n <- length(x)
  Fn <- ecdf(x)
  Gn <- ecdf.leftcont(x)
...
```{r hist_babu}
test_babu <- function(n_tests, n, ar, l) {
  ...
  for (i in 1:n_tests) {
    ...
    results1[i] <- ks.np.babu.ts(x, l, B = 1000)$p.value
    results2[i] <- blkboot(x, ks_statistic, R = 1000, l)$nmp.value
    results3[i] <- ks.test(x, "pnorm")$p.value
    
  }
  
  # Create a master data_frame for p-values
  test <- c(rep("Babu", n_tests), rep("Block", n_tests), rep("Naive", n_tests))
  p_values <- c(results1, results2, results3)
  results <- data_frame(p_values, test)
...
# Create the histograms displaying p-values from each test
results <- suppressWarnings(test_babu(n_tests = n_tests, n = n, ar = ar, l = l))

# Time per test
(proc.time() - ptm)["elapsed"] / n_tests

# Total time
proc.time() - ptm

# Generate histograms plot
hist_babu <- ggplot(results) + 
                geom_histogram(aes(x = p_values, fill = test), boundary = 0, binwidth = 1/20) + 
                scale_x_continuous(name = "p-value", breaks=c(0, 1)) + 
                facet_grid(~test) +
                theme(legend.position = "none")
